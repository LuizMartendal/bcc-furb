...
# Soltar uma peça em uma coluna específica
def soltar_peca(tabuleiro, linha, coluna, peca):
    tabuleiro[linha][coluna] = peca


# Verificar se uma coluna é válida para jogada
def localizacao_valida(tabuleiro, coluna):
    return tabuleiro[LINHAS - 1][coluna] == 0


# Obter a próxima linha disponível em uma coluna
...
# Verificar se um jogador fez um movimento vencedor
def jogada_vencedora(tabuleiro, peca):
    # Verificação horizontal
    for c in range(COLUNAS - 3):
        for l in range(LINHAS):
            if tabuleiro[l][c] == peca and tabuleiro[l][c + 1] == peca and tabuleiro[l][c + 2] == peca and tabuleiro[l][
                c + 3] == peca:
                return True
    # Verificação vertical
    for c in range(COLUNAS):
        for l in range(LINHAS - 3):
            if tabuleiro[l][c] == peca and tabuleiro[l + 1][c] == peca and tabuleiro[l + 2][c] == peca and \
                    tabuleiro[l + 3][c] == peca:
                return True
    # Verificação diagonal positiva
    for c in range(COLUNAS - 3):
        for l in range(LINHAS - 3):
            if tabuleiro[l][c] == peca and tabuleiro[l + 1][c + 1] == peca and tabuleiro[l + 2][c + 2] == peca and \
                    tabuleiro[l + 3][c + 3] == peca:
                return True
...
# Calcular a pontuação de uma posição do tabuleiro
...
# Verificar se o tabuleiro é um nó terminal
def eh_no_terminal(tabuleiro):
    return jogada_vencedora(tabuleiro, PECA_JOGADOR) or jogada_vencedora(tabuleiro, PECA_IA) or (
                len(obter_localizacoes_validas(tabuleiro)) == 0)
...
# Jogada de um jogador, acontece quando se clica em alguma coluna
...
for c in range(COLUNAS):
    botao_coluna = tk.Button(janela, text=str(c), command=lambda c=c: jogador_joga(c), width=5, height=2, fg=COR_BOTAO_LETRA, bg=COR_BOTAO)
